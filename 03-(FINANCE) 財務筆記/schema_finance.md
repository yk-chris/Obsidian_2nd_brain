...
| 資料夾                                      | 使用技能                                                    | 技能路徑                                                       |
| ---------------------------------------- | ------------------------------------------------------- | ---------------------------------------------------------- |
| `03-(FINANCE) 財務筆記/TA技術分析/`              | `invest-stock-valuation.md`                             | `01-(SKILLs) 技能/skill_invest/evaluate_company/`            |
| `03-(FINANCE) 財務筆記/公司業務, 護城河, 週期, 競爭格局/` | `invest-fundamental-analysis.md`                        | `01-(SKILLs) 技能/skill_invest/`                             |
| `03-(FINANCE) 財務筆記/企業概要/`                | `invest-moat-margin-v2.md` + `dive-deep-bottlenecks.md` | `01-(SKILLs) 技能/skill_invest/search_company (background)/` |
| `03-(FINANCE) 財務筆記/產業/`                  | `invest-cost-return.md`                                 | `01-(SKILLs) 技能/skill_invest/search_company (know-how)/`   |
| `03-(FINANCE) 財務筆記/HK-IPO/`              | `invest-IPO.md`                                         | `01-(SKILLs) 技能/skill_invest/`                             |
...
執行 `invest-cost-return` 後，從分析結果中識別 `{ticker}` 的所有 `{業務部門}` 及其 `{貢獻佔比}`，並為**每個業務部門**獨立建立或更新一個筆記檔案，實現業務部門層級的細粒度產業分析。
...
4. **分拆建立或更新檔案** — 針對每個 `{業務部門}`，依下列邏輯判斷操作方式：

   > **存在性判斷規則（以 `{ticker}` + `{業務部門}` 為主鍵）**
   >
   > 掃描 `03-(FINANCE) 財務筆記/產業/` 資料夾，尋找符合以下條件的現有檔案：
   > - 檔名以 `{ticker}_` 開頭，**且**
   > - 檔名以 `_{業務部門}.md` 結尾
   >
   > | 情境 | 判斷結果 | 操作 |
   > |------|----------|------|
   > | 找到符合 `{ticker}_*_{業務部門}.md` 的檔案（貢獻佔比不論） | **已存在** → 更新 | 覆寫該檔案內文，並**重新命名**為新的 `{ticker}_{新貢獻佔比}_{業務部門}.md`（若貢獻佔比有變） |
   > | 找不到符合條件的檔案 | **不存在** → 新建 | 依照 `{ticker}_{貢獻佔比}_{業務部門}.md` 格式新建檔案 |
   >
   > ⚠️ **注意**：貢獻佔比可能隨季報更新而改變，因此不以貢獻佔比判斷存在性，僅以 `{ticker}` + `{業務部門}` 作為唯一識別鍵。
...
> → 掃描 `產業/` 資料夾，判斷每個業務部門是否已存在對應的 `NVDA_*_{業務部門}.md`
> → **已存在** → 覆寫內文並重新命名（若貢獻佔比有變）；**不存在** → 新建
> → 分別建立/更新 `產業/NVDA_58.3_資料中心.md`、`產業/NVDA_22.1_遊戲.md`、`產業/NVDA_9.5_專業視覺化.md` ...
> → 於 `log_finance.md` 記錄每個部門的建立/更新事件
